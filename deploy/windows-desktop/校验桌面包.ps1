...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    Write-Host "[ERROR] PACKAGE_MANIFEST.sha256 不存在。请重新下载完整的 Windows Desktop ZIP。" -ForegroundColor Red
    exit 2
}
...
if ($failed.Count -gt 0) {
    Write-Host "[ERROR] Soda Prompt Hub Windows Desktop 包不完整：" -ForegroundColor Red
