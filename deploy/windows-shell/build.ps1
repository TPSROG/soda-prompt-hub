...
$projectPath = Join-Path $PSScriptRoot "SodaComputeWorker\SodaComputeWorker.csproj"
$releasePath = Join-Path $repositoryRoot "deploy\windows-worker\RELEASE.json"
$release = Get-Content -LiteralPath $releasePath -Raw -Encoding UTF8 | ConvertFrom-Json
$version = [string]$release.worker_version
if ([string]::IsNullOrWhiteSpace($version)) {
    throw "Windows Worker RELEASE.json 缺少 worker_version。"
}

$packageName = "Soda-Compute-Worker-$version-$Runtime"
