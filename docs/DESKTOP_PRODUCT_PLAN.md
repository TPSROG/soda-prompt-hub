...
> 文档状态（2026-09-14）：桌面基础与三端安装包已实施并发布1.1.1预发布候选；高成本原生手验待完成，签名延后。
> 下载与当前行为见[快速开始](QUICK_START.md)，验收见[两模式清单](acceptance/manual-1.1.1-pre1-20260914.md)。
> 历史方案中的A–E阶段、签名目标和检查框不等于当前全部完成；不将历史预期改写为验收证据。
>
> 适用范围：macOS Desktop、Windows Desktop、Windows Compute Worker
>
> 当前代码版本：`1.1.1`；下文保留早期“不改版本号”的历史决策，不作为当前版本说明
