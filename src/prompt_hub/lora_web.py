...
<section class="lora-page" id="loraPage" hidden>
  <section class="lora-hero">
    <div class="lora-hero-copy">
      ...
      <h1>LoRA<br>数据集</h1>
      <p>这里用来确定 LoRA 要学习什么、选择训练图片，并生成数据集副本。Prompt Hub 不会改写 OC Manager、原图或原始说明；筛标、正则和训练仍在 Windows 的 AnimaLoraStudio 中完成。</p>
    </div>
    ...
    <main class="lora-main">
      ...
      <div id="loraProjectDetail" hidden>
        ...
        <section class="lora-panel lora-step-panel" data-lora-panel="5" hidden>
          ...
          <p class="archive-notice">Prompt Hub 只复制已经确认保留的图片和说明文字，不执行训练。Windows AnimaLoraStudio 继续负责最终筛标、正则和训练，旧版本不会被覆盖。</p>
