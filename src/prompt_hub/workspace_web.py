...
<section class="dataset-page" id="workspacePage" hidden>
  <header class="dataset-heading">
    <div><span class="eyebrow">在本机整理</span><h1>数据集工作台</h1></div>
    ...
    <aside class="dataset-sidebar">
      ...
      <form id="datasetImportForm">
        ...
        <details class="dataset-manual-path">
          ...
          <input id="datasetSourcePath" placeholder="__PROMPT_HUB_DATASET_PATH__">
        </details>
        ...
        <p class="dataset-hint">只读取并建立本机工作区，不会移动、改名或覆盖文件夹中的任何内容；上传的 zip 只解压到 Prompt Hub 自己的目录。</p>
      </form>
    ...
    <main class="dataset-main">
      ...
      <div id="datasetDesk" hidden>
        ...
        <section class="dataset-delivery-panel" id="datasetDeliveryPanel" data-dataset-stage-panel="5">
          <header class="dataset-delivery-head"><span class="section-label">05 · 生成交付版本</span><h3>建立可用于训练的数据集副本</h3><p id="datasetDeliverySummary">完成图片和说明文字审核后，这里会显示可交付数量。</p></header>
          <div class="dataset-delivery-grid">
            ...
            <article class="dataset-delivery-action"><strong>保存到本机资料库</strong><p>系统会建立一个新的独立版本，包含图片副本、同名 `.txt` 和校验清单（`manifest.json`、`audit.json`、`hashes.sha256`）。重复点击不会覆盖旧版本，也不修改源文件夹。</p><button class="dataset-primary" id="datasetExportActiveProfile" type="button">生成并保存到资料库</button><p>这里不会启动 Windows 训练；训练、正则和最终筛标继续在 AnimaLoraStudio 中完成。</p><p id="datasetExportResult" class="dataset-hint"></p></article>
          </div>
          <section class="dataset-delivery-history"><div class="dataset-delivery-history-head"><div><span class="section-label">以前生成的版本</span><h4>交付历史</h4></div><p><span>每次生成都会保留独立版本，可下载 ZIP 或打开所在文件夹。</span><span data-usage-only="mac_remote">也可复制到已经挂载的设备：<span data-remote-device-name>__PROMPT_HUB_DEVICE_NAME_HTML__</span>。</span></p></div><div id="datasetDeliveryHistory" class="dataset-delivery-list"><p class="dataset-hint">还没有交付版本。</p></div><p id="datasetCopyResult" class="dataset-hint"></p></section>
        </section>
      </div>
    </main>
  </div>
...
(() => {
  ...
  async function queueWd14(scope) { if (!state.active) return; const paths=scope==='selected'?selectedPaths():[], tagger=$('#datasetTaggerMode').value, model=$('#datasetTaggerModel').value, localTagger=selectedLocalTagger(); if (scope==='selected' && !paths.length) return alert('请先选择要打标的图片。'); if (tagger==='model' && !model) return alert('请先选择用于打标的视觉模型。'); if(tagger==='wd14'&&!localTagger) return alert('请先选择与素材匹配的本地 Tagger。'); if(tagger==='wd14'&&!(await window.ensureLocalTagger(localTagger.id))) return; if (scope==='all' && !confirm('重新打标整个工作区会替换尚未人工确认的 Anima 草稿。继续吗？')) return; const result=await api(`/api/dataset-workspaces/${state.active.workspace_id}/wd14`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope,paths,tagger,model,tagger_model_id:$('#datasetLocalTaggerModel').value,provider:'cpu',overwrite:scope==='all',...captionSettings()})}); renderJob(result.job); startPolling(); }
  ...
  async function saveDetail() {
    ...
    const base=`/api/dataset-workspaces/${state.active.workspace_id}`;
    const status=$('#datasetDetailStatus').value, selected=state.selected.has(item.relative_path);
    const captions={anima:$('#datasetDetailAnima').value.trim(),krea2:$('#datasetDetailKrea2').value.trim()};
    // Persist captions before approving the image. A failed write must not advance review.
    for (const [profile,caption] of Object.entries(captions)) {
      const previous=item.curation?.captions?.[profile] || {};
      const changed=caption!==String(previous.current || '');
      const confirmed=status==='approved' && caption && previous.status!=='reviewed';
      if (changed || confirmed) await api(`${base}/caption`,jsonOptions({relative_path:item.relative_path,profile_id:profile,caption,caption_status:'reviewed'}));
    }
    await api(`${base}/review`,jsonOptions({items:[{relative_path:item.relative_path,status,selected}]}));
    await loadReport(); state.detailIndex=state.pageItems.findIndex(candidate=>candidate.relative_path===item.relative_path); renderDetail();
  }
  ...
  async function refreshInstalledDatasetTaggers() {
    if ($('#workspacePage').hidden) return;
    const select=$('#datasetLocalTaggerModel'), selected=select.value;
    await loadTaggerConfig();
    if (Array.from(select.options).some(option=>option.value===selected)) select.value=selected;
    updateLocalTaggerCalibration();
  }
  window.addEventListener('optional-models-changed',refreshInstalledDatasetTaggers);
