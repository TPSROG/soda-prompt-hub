...
SOURCE_FILES = (
    "LICENSE",
    "pyproject.toml",
    "scripts/package_windows_shell_release.py",
    "scripts/stage_windows_shell_source.py",
)
...
def stage_source(repository_root: Path, output_root: Path) -> dict[str, object]:
    ...
    package_name = f"Soda-Compute-Worker-Source-{version}"
    ...
    manifest_payload = {
        "format": "soda-windows-shell-source-v1",
        "version": version,
        "files": manifest,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage Windows Worker Shell source")
